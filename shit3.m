...
% 5) 交互式多角度选择
% 6) CBF 波束形成提取相位序列
% 7) 原始生命体征处理：unwrap -> 位移(mm) -> movmean
% 8) MSense 运动干扰消除：对每个 target 遍历所有 reference
%    - 论文风格复数除法
%    - 只对 MSense 后序列做 detrend + diff + movmean
% 9) MSense 结果拆成两张图显示，避免位移/差分混绘
...
numLoops = min(size(tx0_data, 3), size(tx2_data, 3));

tx0_data = tx0_data(:, :, 1:numLoops);
tx2_data = tx2_data(:, :, 1:numLoops);
...
loops_per_frame = min(2, numLoops);
...
target_bin = round(target_range_m / detaR_fft) + 1;
target_bin = max(2, min(target_bin, RangFFT));

fprintf('目标距离: %.2f m → 距离bin: %d（分辨率: %.4f m/bin）\n', ...
    target_range_m, target_bin, detaR_fft);
...
for ai = 1:numAngles
    ...
    y_seq = fft_data(:, target_bin);
    angle_y{ai} = y_seq;

    phase_seq_raw = angle(y_seq);
...
figure('Color', 'w', 'Name', '多角度原始生命体征检测结果');
for ai = 1:numAngles
...
if numAngles < 2
    ...
    for ai = 1:numAngles
        ...
        for ri = 1:numAngles
            ...
            % 频域估计前检查长度
            if length(disp_new_mm) < 20
                warning('MSense 序列过短，跳过 target=%.1f°, ref=%.1f°', theta_tar_deg, theta_ref_deg);
                continue;
            end
            ...
            fprintf('    raw范围:  [%.4f, %.4f] mm\n', min(disp_new_mm_raw), max(disp_new_mm_raw));
            fprintf('    proc范围: [%.6f, %.6f]\n', min(disp_new_mm), max(disp_new_mm));
            fprintf('    proc std: %.6f\n', std(disp_new_mm));

            % 时间轴
            time_tar   = (0:length(angle_disp_mm_raw{ai})-1) * T_frame;
            time_ref   = (0:length(angle_disp_mm_raw{ri})-1) * T_frame;
            ...
            %% -------- 图1：原始相关序列 --------
            figure('Color', 'w', 'Name', sprintf('MSense原始_T%.1f_R%.1f', theta_tar_deg, theta_ref_deg));
            sgtitle(sprintf('MSense原始序列：Target %.1f° / Reference %.1f°', ...
                theta_tar_deg, theta_ref_deg), 'FontSize', 13, 'FontWeight', 'bold');

            subplot(3,1,1);
            plot(time_tar, angle_disp_mm_raw{ai}, 'b', 'LineWidth', 1.0);
            grid on;
            xlabel('时间（s）'); ylabel('位移（mm）');
            title(sprintf('Target %.1f° 原始位移', theta_tar_deg));

            subplot(3,1,2);
            plot(time_ref, angle_disp_mm_raw{ri}, 'm', 'LineWidth', 1.0);
            grid on;
            xlabel('时间（s）'); ylabel('位移（mm）');
            title(sprintf('Reference %.1f° 原始位移', theta_ref_deg));

            subplot(3,1,3);
            plot(time_raw, disp_new_mm_raw, 'c', 'LineWidth', 1.1);
            grid on;
            xlabel('时间（s）'); ylabel('位移（mm）');
            title('MSense后原始位移');

            %% -------- 图2：处理后序列 --------
            figure('Color', 'w', 'Name', sprintf('MSense处理后_T%.1f_R%.1f', theta_tar_deg, theta_ref_deg));
            sgtitle(sprintf('MSense处理后：Target %.1f° / Reference %.1f°', ...
                theta_tar_deg, theta_ref_deg), 'FontSize', 13, 'FontWeight', 'bold');

            subplot(4,2,1);
            plot(time_proc, disp_new_mm, 'k', 'LineWidth', 1.1);
            grid on;
            xlabel('时间（s）');
            if enable_diff_msense
                ylabel('差分');
                title('detrend/diff/movmean 后序列');
            else
                ylabel('位移（mm）');
                title('detrend/movmean 后序列');
            end

            subplot(4,2,2);
            plot(f_axis_eval_ms, abs(fft(disp_new_mm, N_fft_eval_ms)), 'k');
            grid on;
            xlabel('频率（Hz）'); ylabel('幅度');
            title('处理后序列FFT');
            xlim([0 3]);
            ...
            subplot(4,2,3);
            plot(time_out, breath_sig, 'g', 'LineWidth', 1.1);
            grid on;
            xlabel('时间（s）'); ylabel('幅度');
            title(sprintf('呼吸时域（%.1f bpm）', br_bpm));

            subplot(4,2,4);
            plot(f_axis_eval_ms, spec_br, 'g', 'LineWidth', 1.0);
            grid on;
            xlabel('频率（Hz）'); ylabel('幅度');
            title('呼吸频谱');
            xlim([0 1]);
            ...
            subplot(4,2,5);
            plot(time_out, heart_sig, 'r', 'LineWidth', 1.1);
            grid on;
            xlabel('时间（s）'); ylabel('幅度');
            title(sprintf('心跳时域（%.1f bpm）', hr_bpm));

            subplot(4,2,6);
            plot(f_axis_eval_ms, spec_hr, 'r', 'LineWidth', 1.0);
            grid on;
            xlabel('频率（Hz）'); ylabel('幅度');
            title('心跳频谱');
            xlim([0 3]);
            ...
            subplot(4,2,7);
            plot(time_raw, disp_new_mm_raw, 'c', 'LineWidth', 1.0); hold on;
            yyaxis right;
            plot(time_proc, disp_new_mm, 'k', 'LineWidth', 1.0);
            grid on;
            xlabel('时间（s）');
            title('MSense原始位移 与 处理后序列（双轴对比）');

            subplot(4,2,8);
            text(0.05, 0.80, sprintf('Target angle = %.1f°', theta_tar_deg), 'FontSize', 11);
            text(0.05, 0.65, sprintf('Reference angle = %.1f°', theta_ref_deg), 'FontSize', 11);
            text(0.05, 0.50, sprintf('Breath = %.1f bpm', br_bpm), 'FontSize', 11);
            text(0.05, 0.35, sprintf('Heart = %.1f bpm', hr_bpm), 'FontSize', 11);
            text(0.05, 0.20, sprintf('Diff enabled = %d', enable_diff_msense), 'FontSize', 11);
            axis off;
        end
    end
